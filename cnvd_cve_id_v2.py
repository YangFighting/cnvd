...
class CnvdSpdier():
    ...
    def get_url_list(self):
        # 读文件,获得URl
        # self.read_txt(file_name='failure_urls.txt')
        self.read_txt()
        # self.url_lines = self.url_lines[0:1]

        self.url_lines = ['https://www.cnvd.org.cn/flaw/show/CNVD-2019-19475']
    ...
    def run(self):
        ...
        for url_i in self.url_lines:
            # 模拟浏览器， 请求URL
            html_dict = self.vulnerability_get(url=url_i)
            print('爬取： %s' % str(url_i).replace('\n', ''))
            try:
                ...
                self.write_failure_url(failure_url=url_i)
                print('爬取失败： 第 %d 个 ' % (self.vulnerability_num + 1))

            if vulnerability_list:
                self.vulnerability_write_excel(write_data_list=vulnerability_list)
